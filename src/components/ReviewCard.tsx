  ...
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        <span
          key={star}
          className="text-lg star-hover select-none"
          style={{ color: star <= rating ? 'hsl(43,96%,50%)' : '#ddd8cc' }}
        >
          ★
        </span>
      ))}
...
export default function ReviewCard({ review, onReact, animationDelay = 0 }: ReviewCardProps) {
  ...
  return (
    <div
      className="bg-white rounded-2xl card-shadow p-5 fade-in transition-shadow duration-300"
    ...
    >
      ...
      <p className="text-sm text-foreground/75 leading-relaxed mb-4 pl-[52px]">
        {review.text}
      </p>

      <div className="flex items-center gap-2 pt-3 border-t border-border/50 pl-[52px]">
        <button
        ...
        >
          <span>👍</span>
          <span>Полезно</span>
          {review.likes > 0 && <span className="font-semibold">{review.likes}</span>}
        </button>
        <button
        ...
        >
          ...
          {review.dislikes > 0 && <span className="font-semibold">{review.dislikes}</span>}
        </button>
      </div>
    </div>
  );
